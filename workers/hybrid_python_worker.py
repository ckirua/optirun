...
def read_frame():
    ...
    if not raw:
        return None
    if len(raw) != 4:
        fail("truncated frame length")
    (n,) = struct.unpack(">I", raw)
    if n < 11 or n > MAX_BODY:
        fail("invalid frame length")
    data = read_exact(n)
    version, typ, request_id = struct.unpack(">HBQ", data[:11])
    if version != 1 or typ not in range(1, 7):
...
def encode_value(value):
    ...
    if isinstance(value, int) and -(1 << 63) <= value < (1 << 63):
        return b"\3" + struct.pack(">q", value)
    if isinstance(value, float):
        return b"\4" + struct.pack(">d", value)
    if isinstance(value, str):
        return b"\5" + string_encode(value)
    if isinstance(value, bytes):
        return b"\6" + struct.pack(">I", len(value)) + value
...
def decode_value(data, offset):
    ...
    if tag in (5, 6):
        ...
        return (raw.decode("utf-8") if tag == 5 else raw), offset + n
...
def load_handlers(path):
    ...
    if not isinstance(handlers, dict) or not handlers:
        fail("HANDLERS must be a nonempty dictionary")
    if any(
        not isinstance(k, str) or not k or not callable(v) for k, v in handlers.items()
    ):
        fail("invalid HANDLERS entry")
    return dict(sorted(handlers.items()))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--handler-file", required=True)
    ...
    hello = json.dumps(
        {
            ...
            "build_supports_free_threading": sysconfig.get_config_var("Py_GIL_DISABLED")
            == 1,
    ...
    while True:
        ...
        if typ == SHUTDOWN:
            if body:
                fail("shutdown payload")
            write_frame(SHUTDOWN_ACK, request_id)
            return
        if typ != INVOKE:
            fail("unexpected message type")
        try:
            if len(body) < 6:
                raise ValueError("truncated invocation")
            handler_id, argc = struct.unpack(">IH", body[:6])
            offset = 6
            names = list(handlers)
            if handler_id == 0 or handler_id > len(names):
            ...
            for _ in range(argc):
                ...
                values.append(value)
            if offset != len(body):
                raise ValueError("trailing invocation data")
            write_frame(
                RESULT,
                request_id,
                encode_value(handlers[names[handler_id - 1]](*values)),
            )
        except BaseException as exc:
            write_frame(
                ERROR,
                request_id,
                string_encode(type(exc).__name__)
                + string_encode(str(exc))
                + string_encode(traceback.format_exc()),
            )
